...
def generate_model(dataset, run):
    ...
    cf_matrix_json = {
       ...
       "schema_version": "v1",
       "data": {
           "class_labels": group_names,
           "matrix": cf_matrix.tolist()
        }
     }
    run.log_confusion_matrix('Employee Retention Confusion Matrix', cf_matrix_json, description='Confusion matrix generated for the run')

    # Log the following metrics to the parent run so that these are available for model evaluation later.
...
def main():
    ...
    parser.add_argument('--clean_data_folder', dest='clean_data_folder', type=str, help='folder path that stores the clean data file')
    parser.add_argument('--new_model_folder', dest='new_model_folder', type=str, help='output folder path that stores the new model .pkl file name')
    parser.add_argument('--clean_data_file', dest='clean_data_file', type=str, help='name of the clean data file')
    parser.add_argument('--new_model_file', dest='new_model_file', type=str, help='name of the new model .pkl file')
    ...
    # For "output" PipelineData, the folder must be created using os.makedirs() first, then only can write files into the folder.
