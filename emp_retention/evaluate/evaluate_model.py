...
def main():
    ...
    if(model is not None):
        if(new_metrics['Accuracy'] >= current_metrics['Accuracy']):
            run.log("Result", "New Accuracy is as good as current, will proceed to register new model.")
        else:
            run.log("Result", "New Accuracy is worse than current, will not register model. Processing cancelled.")
            run.parent.cancel()
    else:
